...
# the hms_foil_rates_carbon_inelastic_multifoil macro, producing some histograms and getting the rate of events originating from each.
if ( $#argv != 4 ) then
    ...
    if (-e "$pth") then
        ...
        .x examples/hms_foil_rates_carbon_inelastic_multifoil.C("$argv[1]", $argv[2], $argv[3], $argv[4])
        .q
        EOF
    else
